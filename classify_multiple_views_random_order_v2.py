...
class ImageClassificationApp:
    def __init__(
        ...
        self.panel_locked = False  # Track if the current panel is locked (selection made)
    ...
    def create_button_frames(self):
        ...
        # Main container frame with FIXED HEIGHT
        self.button_frame = ttk.Frame(self.master)
        self.button_frame.grid(row=1, column=0, columnspan=3, sticky='ew', padx=20)

        # Set a minimum height for button_frame to prevent shifting
        self.button_frame.grid_propagate(False)  # Prevent automatic resizing
        self.button_frame.configure(height=280)
        ...
        # Create panels container with FIXED HEIGHT
        panels_container = ttk.Frame(self.button_frame)
        panels_container.grid(row=1, column=1, sticky='ew')
        panels_container.configure(height=230)
        panels_container.grid_propagate(False)  # Enforce fixed height

        # Create comment box frame with FIXED DIMENSIONS
        comment_frame = ttk.LabelFrame(self.button_frame, text='Comments')
        comment_frame.grid(row=1, column=2, sticky='nsew', padx=(20, 0))
        comment_frame.configure(width=200, height=230)
        comment_frame.grid_propagate(False)  # Enforce fixed dimensions

        # Create and configure the comment box with FIXED SIZE
        ...
        # Create panels with FIXED OUTER DIMENSIONS
        # The key is to use place manager for the buttons inside each panel
        # to ensure they don't move when the panel's border changes

        # Primary panel (Dwarf classification)
        self.primary_panel = ttk.LabelFrame(panels_container, text='Is this a dwarf galaxy?')
        self.primary_panel.pack(fill='x', pady=(0, 5))
        self.primary_panel.configure(height=70)  # Fixed height
        ...
        if self.with_morphology:
            ...
            self.morphology_panel.configure(height=70)  # Fixed height
        ...
        self.special_features_panel.configure(height=70)  # Fixed height
    ...
    def create_dwarf_buttons(self):
        ...
        primary_buttons_frame.pack(fill='x', padx=20, pady=5)
    ...
    def create_morphology_buttons(self):
        ...
        morph_buttons_frame.pack(fill='x', padx=20, pady=5)
    ...
    def create_special_features_buttons(self):
        ...
        special_features_frame.pack(fill='x', padx=20, pady=5)
    ...
    def set_special_feature(self, feature):
        """Set the current special feature selection and auto-save after delay"""
        # Lock the special features panel
        self.panel_locked = True
        ...
        selected_button = self.special_features_buttons[feature]
        selected_button.configure(style='Selected.TButton')

        # Grey out the special features panel
        self.special_features_panel.configure(style='Inactive.TLabelframe')

        # Disable buttons but maintain pressed appearance
        for btn in self.special_features_buttons.values():
            btn.state(['disabled'])
    ...
    def reset_special_features(self):
        ...
        self.current_special_feature = None  # No default selection
        for btn in self.special_features_buttons.values():
            btn.configure(style='TButton')
    ...
    def handle_key_press(self, event):
        ...
        if (
            event.widget == self.comment_box
            or self.random_index_ptr >= len(self.unclassified_indices)
            or self.panel_locked
        ):  # Add check for locked panel
            return
    ...
    def handle_enter(self, event):
        """Handle Enter key press in comment box"""
        # Only prevent default behavior if in comment box
        if event.widget == self.comment_box:
            return 'break'  # Prevent default line break behavior
    ...
    def set_morphology(self, morphology):
        ...
        # Highlight selected button
        selected_button = self.morph_buttons[morphology]
        selected_button.configure(style='Selected.TButton')

        # Grey out morphology panel
        ...
        # Transition to special features classification without pre-selecting
        self.current_classification_mode = 'special_features'
        self.current_special_feature = None  # No default selection

        # Reset panel_locked when moving to special features panel
        self.panel_locked = False

        # Update panel states to activate special features panel
        self.update_panel_states('special_features')

        # Update status message
        self.status_label.configure(text='Currently classifying: Special features')

    def handle_classification(self, value):
        """Handle dwarf classification"""
        # Set panel as locked immediately
        self.panel_locked = True
        ...
        # Highlight selected button and determine which one was pressed
        if value == 0:  # No Dwarf
            selected_button = self.no_dwarf_button
        elif value == 0.5:  # Maybe Dwarf
            selected_button = self.maybe_dwarf_button
        else:  # Dwarf
            selected_button = self.dwarf_button

        # Apply the selected style to make button appear pressed
        selected_button.configure(style='Selected.TButton')
        ...
        if value == 0:  # No Dwarf
            ...
            if not self.with_morphology:
                ...
                # Reset panel_locked when moving to the next question
                self.panel_locked = False
    ...
    def setup_styles(self):
        """Setup custom styles for the panels and buttons with visual feedback"""
        style = ttk.Style()

        # Panel styles
        # Active panel - thicker border (2px) with less internal padding
        style.configure(
            'Active.TLabelframe',
            background='white',
            borderwidth=2,  # Thicker border
            relief='solid',
            padding=4,  # Less internal padding
        )
        style.configure(
        ...
        style.map(
            'Active.TLabelframe',
            bordercolor=[('!disabled', '#007bff')],
        )

        # Inactive panel - thinner border (1px) but MORE internal padding to compensate
        style.configure(
            'Inactive.TLabelframe',
            background='gray90',
            borderwidth=1,  # Thinner border
            relief='solid',
            padding=5,  # MORE padding to compensate (1px more)
        )
        ...
        # Disabled panel - same as inactive
        style.configure(
        ...
        # Button styles
        button_width = 15  # Fixed width for all states
        ...
        style.configure(
            ...
            relief='raised',
            borderwidth=2,
            padding=(4, 4, 4, 4),  # Symmetric padding
        )

        # Selected button style - with blue background
        style.configure(
            'Selected.TButton',
            width=button_width,
            relief='sunken',
            ...
            padding=(6, 2, 2, 6),  # Adjusted padding for sunken appearance
        )

        # Selected and disabled button style
        style.configure(
            'Selected.Disabled.TButton',
            width=button_width,
            relief='sunken',
            borderwidth=2,
            background='#ADD8E6',  # Slightly dimmed lightblue
            foreground='gray30',
            padding=(6, 2, 2, 6),
        )

        # FIX FOR HOVER ISSUE: Ensure 'active' state doesn't override selection
        style.map(
            'Selected.TButton',
            background=[('active', 'lightblue')],  # Keep blue background when hovering
            relief=[('active', 'sunken')],  # Keep sunken relief when hovering
            padding=[('active', (6, 2, 2, 6))],  # Keep the same padding when hovering
        )
    ...
    def save_classification(self):
        ...
        # Reset the panel_locked flag for the new image
        self.panel_locked = False
